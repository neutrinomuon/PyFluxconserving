...
def spectral_resampling( new_wavs, spec_wavs, spec_fluxes, spec_errs=None, fill=None, verbose=True ):
    ...
    # Modified by Jean
    #print(old_edges[-1],new_edges)
    ind_last_edge = np.argwhere(new_edges>old_edges[-1])
    #print(np.size(ind_last_edge))
    if np.size(ind_last_edge)>0:
        new_edges[ind_last_edge[0]-1] = old_edges[-1]+1e-15
    
    ind_first_edge = np.argwhere(new_edges<old_edges[0])
    #print(ind_first_edge)
    
    #if np.size(ind_first_edge)>0:
     #   new_edges[ind_first_edge[-1]+1] = old_edges[0]+1e-15
