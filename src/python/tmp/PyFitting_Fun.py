...
import numpy as np
import Fitting_Fun as interp
import scipy.interpolate as interp1
import AkimaSpline as interp2
...
"""
    ...
    def TestRoutine( yydegree=3 ):
        ...
        y_interp, IsKeepOn = interp.fitting_fun.poly_interp( x_interp, x, y, yydegree=yydegree, verbosity=verbosity )
        y_new_interp, IsKeepOn = interp.fitting_fun.poly_interp( x_new_interp, x_new, y_new, yydegree=yydegree, verbosity=verbosity )

        #print(y_interp)
        #print(y_new_interp)

        # Akima using the Python3 interface
        y_interp_Akima = akima_interp( x_interp, x, y, delta__x=delta__x )
        y_new_interp_Akima = akima_interp( x_new_interp, x_new, y_new, delta__x=delta__x )
        ...
        coeff = np.polyfit(x, y, yydegree) 
        print(' ==== ')
        coeff_new = np.polyfit(x_new, y_new, yydegree) 
        print(x_new.size)
        
        y_interp_numpy = np.zeros([np.size(x_interp)])
        y_new_interp_numpy = np.zeros([np.size(x_new_interp)])

        for i in enumerate(coeff):
            y_interp_numpy += i[1] * x_interp**(yydegree-i[0])
            
        for i in enumerate(coeff_new):
             y_new_interp_numpy += i[1] * x_new_interp**(yydegree-i[0])
        ...
        # Original values
        ax1_top.plot(x,y,color='darkorange',linewidth=10,label='Original',zorder=1)
        ax1_top.scatter(x,y,color='blue',s=120,zorder=2)

        # Interpolated function Fortran - f2py
        ax1_top.plot(x_interp, y_interp, color='blue',label='LAPACK Polynomial Interpolation',linewidth=10,alpha=0.3)

        # Numpy polynomial interpolation 
        ax1_top.plot(x_interp, y_interp_numpy, color='darkgreen',label='Numpy Polynomial Interpolation',linewidth=2,alpha=1)
        ...
        # Interpolated function Fortran - f2py
        ax2_top.plot(x_new_interp, y_new_interp, color='blue',label='LAPACK Polynomial Interpolation',linewidth=10,alpha=0.3)
        
        # Numpy polynomial interpolation 
        ax2_top.plot(x_new_interp, y_new_interp_numpy, color='darkgreen',label='Numpy Polynomial Interpolation',linewidth=2,alpha=1)
    ...
    def time():
        ...
        for i in enumerate(val_array):
            ...
            for pop in range(0,population):
                ...
                print("... Time LAPACK Fortran: {0:} s ".format(auxil2time-auxil1time))
        ...
        plt.plot(sizepoints,timearray_average[:,0],color='darkorange',label='Time LAPACK Fortran', zorder=1)
...
def akima_interp( x, xpt, ypt, delta__x=0.1 ):
    ...
    p0, p1, p2, p3 = interp2.akima_setup( xpt, ypt, delta__x=delta__x )
    ...
    y, dydx, dydxpt, dydypt = interp2.akimainterp( x, xpt, p0, p1, p2, p3, zeros, zeros, zeros, zeros, zeros, zeros, zeros, zeros )
...
def main():
    i_object = poly_interp
    
    yydegree = 12
    i_object.TestRoutine( yydegree=yydegree )
    ...
    # #i_object = AkimaSpline( x_new,x,y,tipo=1 )
        
    # #print( i_object.y_new )
    
    # o_object = Akima( x,y,delta_x=.400 )
