...
import numpy as np
import SPLINE3DFor as interp2

import SPLINECubic as interp
...
'''
    ...
    def TestRoutine():
        ...
        y2,iskeepon = interp.spline_diff( x,y,yp1=yp1,ypn=ypn )        
        y2_new,iskeepon =  interp.spline_diff( x_new,y_new,yp1=yp1,ypn=ypn )
        ...
        #y_new_interp,IsKeepOn = interp.spline3darr(x_new_interp,x_new,y_new,verbosity=verbosity)
        
        s1 = time.time()
        y_interp = np.zeros([x_interp.size])
        ilastval = -1
        for i in enumerate(x_interp):
            y_interp[i[0]], __ = interp.spline__int( i[1],x,y,y2,ilastval )
            ilastval += 1
        e1 = time.time()
        
        s2 = time.time()
        y_new_interp = np.zeros([x_new_interp.size])
        ilastval = -1
        for i in enumerate(x_new_interp):
            y_new_interp[i[0]], __ = interp.spline__int( i[1],x_new,y_new,y2_new,ilastval )
            ilastval += 1
        e2 = time.time()
        
        s3 = time.time()
        y_interp, __ = interp.splinecubic( x_interp,x,y )
        e3 = time.time()
        
        s4 = time.time()
        y_new_interp, __ = interp.splinecubic( x_new_interp,x_new,y_new )
        e4 = time.time()
        
        print( e1-s1,e3-s3,(e3-s3)/(e1-s1) )
        print( e2-s2,e4-s4,(e4-s4)/(e2-s2) )
    ...
    def time():
        ...
        population = 10
        timearray  = np.zeros([i_,4,population])
        timearray_average  = np.zeros([i_,4])
        standard_deviation = np.zeros([i_,4])
        ...
        for i in enumerate(val_array):
            ...
            for pop in range(0,population):
                ...
                # Calling directly the f2py subroutine
                auxil1time_ = time.time()
                y_interp,IsKeepOn = interp2.spline3darr(x_interp,x,y,verbosity=verbosity)
                auxil2time_ = time.time()
                
                auxil1time = time.time()
                y_interp,IsKeepOn = interp.splinecubic(x_interp,x,y,verbosity=verbosity)
                ...
                timearray[i[0],3,pop] = auxil2time_-auxil1time_
                ...
                timearray_average[i[0],3]  += timearray[i[0],3,pop]
                
                print("... Time    SPLINECubic: {0:} s ".format(auxil2time-auxil1time))
                print("... Time    Spline3DFor: {0:} s ".format(auxil2time_-auxil1time_))
            ...
            timearray_average[i[0],3]  /= population 
            ...
            standard_deviation[i[0],3] = np.std( timearray[i[0],3,:] )
        ...
        plt.plot(sizepoints,timearray_average[:,0],color='darkorange',label='Time SPLINECubic Fortran', zorder=1)
        ...
        plt.plot(sizepoints,timearray_average[:,3],color='red',label='Time SPLINE3DFor  Spline', zorder=2)
        

        plt.plot(sizepoints,timearray_average[:,1],color='darkgreen',label='Time Akima Scipy', zorder=3)
        ...
        plt.plot(sizepoints,timearray_average[:,2],color='#1f77b4',label='Time Scipy  Spline',zorder=4)
