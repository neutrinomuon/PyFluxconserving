...
! ###########################################################################
!     RESUME : A package for piecewise cubic Hermite interpolation of
!              the data. In numerical analysis, Hermite interpolation,
!              named after Charles Hermite, is a method of polynomial
!              interpolation, which generalizes Lagrange
!              interpolation. Lagrange interpolation allows computing
!              a polynomial of degree less than n that takes the same
!              value at n given points as a given function. Instead,
!              Hermite interpolation computes a polynomial of degree
!              less than mn such that the polynomial and its m − 1
!              first derivatives have the same values at n given
!              points as a given function and its m − 1 first
!              derivatives.

!              Hermite's method of interpolation is closely related to
!              the Newton's interpolation method, in that both are
...
MODULE pchipmodule
    ...
    ! Private
    !integer  (kind=IB), parameter :: wp=8
    
    ! Numerical Constants:
    real     (kind=RP), parameter :: zero    = 00.0_RP
    real     (kind=RP), parameter :: half    = 00.5_RP
    real     (kind=RP), parameter :: one     = 01.0_RP
    real     (kind=RP), parameter :: two     = 02.0_RP
    real     (kind=RP), parameter :: three   = 03.0_RP
    real     (kind=RP), parameter :: four    = 04.0_RP
    real     (kind=RP), parameter :: six     = 06.0_RP
    real     (kind=RP), parameter :: ten     = 10.0_RP
    real     (kind=RP), parameter :: d1mach4 = epsilon(one)  ! d1mach(4) -- The largest relative spacing

    ! Parameter
    integer  (kind=IB), parameter :: zeroint  = 0_IB
    integer  (kind=IB), parameter :: one_int  = 1_IB
    integer  (kind=IB), parameter :: two_int  = 2_IB
    integer  (kind=IB), parameter :: threeint = 3_IB
    
    ! Public routines:
    !public :: dpchim,dpchic,dpchsp
    !public :: dchfev,dpchfe,dchfdv,dpchfd,dpchid,dpchia
    !public :: dpchbs,dpchcm
    !public :: dpchsd,dchfcm
...
PURE FUNCTION dpchsd( s1,s2,h1,h2 )
    ...
    real     (kind=RP), intent(in) :: s1, s2, h1, h2
    real     (kind=RP) :: dpchsd
...
!
!  Called by [[DPCHCM]] toc determine the monotonicity properties of the
...
FUNCTION dchfcm( d1,d2,delta ) RESULT( ismon )
    ...
    real     (kind=RP), intent(in) :: d1    !! Derivative value at the end of an interval.
    real     (kind=RP), intent(in) :: d2    !! Derivative value at the end of an interval.
    real     (kind=RP), intent(in) :: delta !! The data slope over that interval
    integer  (kind=IB) :: ismon             !! Indicates the monotonicity of the cubic segment:
                                            !!
    ...
    integer  (kind=IB) :: itrue
    real     (kind=RP) :: a,b,phi
    
    real     (kind=RP), parameter :: eps = ten*d1mach4 !! machine-dependent parameter -- should be about 10*uround.
...
SUBROUTINE dchfdv( x1,x2,f1,f2,d1,d2,ne,xe,fe,de,next,ierr )
    ...
    integer  (kind=IB), intent(in)                :: ne !! number of evaluation points.  (Error return if NE<1).
    real     (kind=RP),dimension(*), intent(in)  :: xe !! array of points at which the functions are to
                                            !! be evaluated.  If any of the XE are outside the interval
                                            !! [X1,X2], a warning error is returned in NEXT.
    real     (kind=RP),dimension(*), intent(out) :: fe !! array of values of the cubic function
                                            !! defined by X1,X2, F1,F2, D1,D2 at the points XE.
    real     (kind=RP),dimension(*), intent(out) :: de !! array of values of the first derivative of
                                            !! the same function at the points XE.
    real     (kind=RP), intent(in)               :: x1 !! initial endpoint of interval of definition of cubic. (Error return if X1==X2.)
    real     (kind=RP), intent(in)               :: x2 !! final endpoint of interval of definition of cubic. (Error return if X1==X2.)
    real     (kind=RP), intent(in)               :: f1 !! value of function at X1.
    real     (kind=RP), intent(in)               :: f2 !! value of function at X2.
    real     (kind=RP), intent(in)               :: d1 !! value of derivative at X1.
    real     (kind=RP), intent(in)               :: d2 !! value of derivative at X2.
    integer  (kind=IB),dimension(2), intent(out) :: next !! array indicating number of extrapolation points:
                                             !!
                                             !! * NEXT(1) = number of evaluation points to left of interval.
                                             !! * NEXT(2) = number of evaluation points to right of interval.
    integer  (kind=IB), intent(out) :: ierr     !! error flag.
                                    !!
                                    !! Normal return:
                                    !!
                                    !! * IERR = 0  (no errors).
    ...
    integer  (kind=IB) :: i
    real     (kind=RP) :: c2, c2t2, c3, c3t3, del1, del2, delta, h, x, xmi, xma

    ! validity-check arguments.
    if ( ne < 1 ) then
       ierr = -1
       call xermsg ('PCHIP', 'dchfdv', 'number of evaluation points less than one', ierr, one_int)
       return
    end if
    h = x2 - x1
    if (h == zero) then
       ierr = -2
       call xermsg ('PCHIP', 'dchfdv', 'interval endpoints equal', ierr, one_int)
       return
    end if
...
subroutine dchfev ( x1,x2,f1,f2,d1,d2,ne,xe,fe,next,ierr )
    ...
    integer  (kind=IB), intent(in)                :: ne     !! number of evaluation points.  (Error return if NE<1).
    integer  (kind=IB),dimension(2), intent(out)  :: next   !! integer  (kind=IB) array indicating number of extrapolation points:
                                                !!
                                                !! * NEXT(1) = number of evaluation points to left of interval.
                                                !! * NEXT(2) = number of evaluation points to right of interval.
    integer  (kind=IB), intent(out)               :: ierr   !! error flag.
    ...
    real     (kind=RP), intent(in)               :: d1     !! value of derivative at X1.
    real     (kind=RP), intent(in)               :: d2     !! value of derivative at X2.
    real     (kind=RP),dimension(ne), intent(in)  :: xe     !! array of points at which the function is to
                                                !! be evaluated. If any of the XE are outside the interval
                                                !! [X1,X2], a warning error is returned in NEXT.
    real     (kind=RP),dimension(ne), intent(out) :: fe     !! array of values of the cubic function
                                                !! defined by X1,X2, F1,F2, D1,D2 at the points XE.

    integer  (kind=IB) :: i
    real     (kind=RP) :: c2, c3, del1, del2, delta, h, x, xmi, xma

    !f2py integer  (kind=IB), intent(out) :: ierr
    !f2py integer  (kind=IB), intent(out) :: next
    
    !f2py real     (kind=RP), intent(in)  :: x1,x2,f1,f2,d1,d2
    !f2py real     (kind=RP), intent(in)  :: xe
    !f2py real     (kind=RP), intent(out) :: fe
    !f2py                     intent(hide), depend(xe) :: ne=shape(xe,0)
    !f2py                     intent(hide), depend(fe) :: ne=shape(fe,0)
    
    ! validity-check arguments.
    if (ne < 1) then
        ierr = -1
        call xermsg ('PCHIP', 'dchfev', 'number of evaluation points less than one', ierr, one_int)
        return
    end if
    h = x2 - x1
    if (h == zero) then
        ierr = -2
        call xermsg ('PCHIP', 'dchfev', 'interval endpoints equal', ierr, one_int)
        return
    end if
...
!      indeed the mathematically correct answer when X1==X2 .
    ...
    real     (kind=RP), intent(in) :: x1 !! endpoints if interval of definition of cubic
    real     (kind=RP), intent(in) :: x2 !! endpoints if interval of definition of cubic
    real     (kind=RP), intent(in) :: f1 !! function values at the ends of the interval
    real     (kind=RP), intent(in) :: f2 !! function values at the ends of the interval
    real     (kind=RP), intent(in) :: d1 !! derivative values at the ends of the interval
    real     (kind=RP), intent(in) :: d2 !! derivative values at the ends of the interval
    real     (kind=RP), intent(in) :: a  !! endpoints of interval of integration
    real     (kind=RP), intent(in) :: b  !! endpoints of interval of integration
    real     (kind=RP) :: integral

    real     (kind=RP) :: dterm, fterm, h, phia1, phia2, &
...
!    pp.79-82.
    ...
    integer  (kind=IB), intent(in)     :: n               !! the number of data points, N>=2 .  (not checked)
    integer  (kind=IB), intent(in)     :: incfd           !! the increment between successive values in F and D.
    ...
    integer  (kind=IB), intent(in)     :: knotyp          !! a flag to control the knot sequence.
    ...
    integer  (kind=IB), intent(inout)  :: nknots          !! the number of knots.
                                              !!
    ...
    integer  (kind=IB), intent(out)    :: ndim            !! the dimension of the B-spline space.  (Set to 2*N.)
    integer  (kind=IB), intent(out)    :: kord            !! the order of the B-spline.  (Set to 4.)
    integer  (kind=IB), intent(out)    :: ierr            !! an error flag:
                                              !!
                                              !! * Normal return:
                                              !!    * IERR = 0  (no errors).
                                              !! * "Recoverable" errors:
    ...
    real     (kind=RP), intent(in)    :: x(*)            !! array of independent variable values.  The
                                              !! elements of X must be strictly increasing:
                                              !!      X(I-1) < X(I),  I = 2(1)N.   (not checked)
                                              !! nmax, the dimension of X, must be >=N.
    real     (kind=RP), intent(in)    :: f(incfd,*)      !! array of dependent variable values.
                                              !! F(1+(I-1)*INCFD) is the value corresponding to X(I).
                                              !! nmax, the second dimension of F, must be >=N.
    real     (kind=RP), intent(in)    :: d(incfd,*)      !! array of derivative values at the data points.
                                              !! D(1+(I-1)*INCFD) is the value corresponding to X(I).
                                              !! nmax, the second dimension of D, must be >=N.
    real     (kind=RP), intent(inout) :: t(*)            !! array of 2*N+4 knots for the B-representation.
                                              !!
    ...
    real     (kind=RP), intent(out)   :: bcoef(*)        !! array of 2*N B-spline coefficients.

    integer  (kind=IB) :: k, kk
    real     (kind=RP) :: dov3, hnew, hold
    ...
    if ( knotyp>2 ) then
        ierr = -1
        call xermsg ('PCHIP', 'dpchbs', 'knotyp greater than 2', ierr, one_int)
        return
    endif
    if ( knotyp<0 ) then
        if ( nknots/=ndim+4 ) then
            ierr = -2
            call xermsg ('PCHIP', 'dpchbs', 'knotyp<0 and nknots/=(2*n+4)', ierr, one_int)
            return
        endif
    else
...
!@warning This routine does no validity-checking of arguments.
    ...
    integer  (kind=IB), intent(in)  :: ic(2)    !! array of length 2 specifying desired
                                    !! boundary conditions:
                                    !!
    ...
    integer  (kind=IB), intent(in)     :: n     !! number of data points.  (assumes N>=2)
    integer  (kind=IB), intent(in)     :: incfd !! increment between successive values in D.
                                    !! This argument is provided primarily for 2-D applications.
    integer  (kind=IB), intent(out)    :: ierr  !! error flag.
                                    !!
                                    !! * Normal return:
    ...
    real     (kind=RP), intent(in)    :: vc(2) !! array of length 2 specifying desired boundary
                                    !! values.
                                    !!
    ...
    real     (kind=RP), intent(in)    :: h(*)  !! array of interval lengths.
    real     (kind=RP), intent(in)    :: slope(*)  !! array of data slopes.
                                        !! If the data are (X(I),Y(I)), I=1(1)N, then these inputs are:
                                        !!
                                        !! * H(I) =  X(I+1)-X(I),
                                        !! * SLOPE(I) = (Y(I+1)-Y(I))/H(I),  I=1(1)N-1.
    real     (kind=RP), intent(inout) :: d(incfd,*) !! (input) real*8 array of derivative values at the data points.
    ...
    integer  (kind=IB) :: ibeg, iend, ierf, index, j, k
    real     (kind=RP) :: stemp(3), xtemp(4)
    ...
    if (ibeg /= 0) then
        ...
        if (k == 1) then
            ...
            if (ierf /= 0) then
                ...
                ierr = -1
                call xermsg ('PCHIP', 'dpchce', 'error return from dpchdf', ierr, one_int)
                return
            end if
        else
...
!@warning This routine does no validity-checking of arguments.
    ...
    integer  (kind=IB), intent(in)   :: n           !! number of data points.
    ...
    real     (kind=RP), intent(in)  :: slope(*)    !! array of data slopes.
                                        ...
                                        !! * H(I) =  X(I+1)-X(I),
                                        !! * SLOPE(I) = (Y(I+1)-Y(I))/H(I),  I=1(1)N-1.
    real     (kind=RP), intent(out) :: d(incfd,*)  !! array of derivative values at data points.
    ...
    integer  (kind=IB) :: i, nless1
    real     (kind=RP) :: del1, del2, dmax, dmin, drat1, drat2, hsum, hsumt3, w1, w2
...
!  ISMON(N) is ever equal to 2, for it cannot be changed further.
    ...
    integer  (kind=IB), intent(in)    :: n          !! the number of data points.  (Error return if N<2).
    real     (kind=RP), intent(in)   :: x(n)       !! array of independent variable values.  The
                                        !! elements of X must be strictly increasing:
                                        !!      X(I-1) < X(I),  I = 2(1)N.
                                        !! (Error return if not.)
    real     (kind=RP), intent(in)   :: f(incfd,n) !! array of function values.  F(1+(I-1)*INCFD) is
                                        !! the value corresponding to X(I).
    real     (kind=RP), intent(in)   :: d(incfd,n) !! array of derivative values.  D(1+(I-1)*INCFD) is
    ...
    integer  (kind=IB), intent(out)   :: ismon(n)   !! array indicating on which intervals the
    ...
    if (.not. skip) then
        if ( n<2 ) then
            ierr = -1
            call xermsg ('PCHIP', 'dpchcm', 'number of data points less than two', ierr, one_int)
            return
        end if
        if ( incfd<1 ) then
            ierr = -2
            call xermsg ('PCHIP', 'dpchcm', 'increment less than one', ierr, one_int)
            return
        end if
        do i = 2, n
            if ( x(i)<=x(i-1) ) then
                ...
                ierr = -3
                call xermsg ('PCHIP', 'dpchcm', 'x-array not strictly increasing', ierr, one_int)
                return
            end if
        end do
        skip = .true.
...
!     -1 if they are of opposite sign.
    ...
    real     (kind=RP), intent(in)   :: switch      !! indicates the amount of control desired over
    ...
    real     (kind=RP), intent(in)   :: slope(*)    !! array of data slopes.
                                         ...
                                         !! * H(I) =  X(I+1)-X(I),
                                         !! * SLOPE(I) = (Y(I+1)-Y(I))/H(I),  I=1(1)N-1.
    real     (kind=RP), intent(inout) :: d(incfd,*) !! (input) array of derivative values at the data points,
    ...
    integer  (kind=IB), intent(in)    :: incfd       !! increment between successive values in D.
                                         !! This argument is provided primarily for 2-D applications.
    integer  (kind=IB), intent(out)   :: ierr        !! error flag.  should be zero.
                                         !! If negative, trouble in [[DPCHSW]].  (should never happen.)

    integer  (kind=IB) :: i, indx, k, nless1
    real     (kind=RP) :: del(3), dext, dfloc, dfmx, fact, slmax, wtave(2)

    real     (kind=RP),parameter :: fudge = four
...
!    New York, 1978, pp. 10-16.
    ...
    integer  (kind=IB), intent(in)     :: k    !! is the order of the desired derivative approximation.
                                   !! K must be at least 3 (error return if not).
    real     (kind=RP), intent(in)    :: x(k) !! contains the K values of the independent variable.
                                   !! X need not be ordered, but the values **MUST** be
                                   !! distinct.  (Not checked here.)
    real     (kind=RP), intent(inout) :: s(k) !! contains the associated slope values:
                                   !! `S(I) = (F(I+1)-F(I))/(X(I+1)-X(I)), I=1(1)K-1`.
                                   !! (Note that S need only be of length K-1.)
                                   !! Will be destroyed on output.
    integer  (kind=IB), intent(out)    :: ierr !! will be set to -1 if K<2 .
    real     (kind=RP) :: deriv  !! will be set to the desired derivative approximation if
    ...
    if (k < 3) then
        ierr = -1
        call xermsg ('PCHIP', 'dpchdf', 'k less than three', ierr, one_int)
        deriv = zero
    else
...
!    will be counted (at least) twice in the total returned in IERR.
    ...
    integer  (kind=IB), intent(in)    :: n          !! number of data points.  (Error return if N<2).
    integer  (kind=IB), intent(in)    :: incfd      !! increment between successive values in F and D.
                                        !! (Error return if INCFD<1).
    integer  (kind=IB), intent(in)    :: ne         !! number of evaluation points.  (Error return if NE<1).
    integer  (kind=IB), intent(out)   :: ierr       !! error flag.
                                        !!
    ...
    real     (kind=RP), intent(in)   :: x(*)       !! array of independent variable values.  The
                                        ...
                                        !! `X(I-1) < X(I),  I = 2(1)N`.
                                        !! (Error return if not.)
    real     (kind=RP), intent(in)   :: f(incfd,*) !! array of function values.  F(1+(I-1)*INCFD) is
                                        !! the value corresponding to X(I).
    real     (kind=RP), intent(in)   :: d(incfd,*) !! array of derivative values.  D(1+(I-1)*INCFD)
                                        !! is the value corresponding to X(I).
    real     (kind=RP), intent(in)   :: xe(*)      !! array of points at which the functions are to
                                        !! be evaluated.
                                        !! NOTES:
                                        !!
    ...
    real     (kind=RP), intent(out)  :: fe(*)      !! array of values of the cubic Hermite
                                        !! function defined by  N, X, F, D at the points XE.
    real     (kind=RP), intent(out)  :: de(*)      !! array of values of the first derivative of
                                        !! the same function at the points XE.
    logical, intent(inout) :: skip       !! logical variable which should be set to
                                        !! .TRUE. if the user wishes to skip checks for validity of
                                        !! preceding parameters, or to .FALSE. otherwise.
    ...
    integer  (kind=IB) :: i, ierc, ir, j, jfirst, next(2), nj
    ...
    if (.not. skip) then
        if ( n<2 ) then
            ierr = -1
            call xermsg ('PCHIP', 'dpchfd', 'number of data points less than two', ierr, one_int)
            return
        end if
        if ( incfd<1 ) then
            ierr = -2
            call xermsg ('PCHIP', 'dpchfd', 'increment less than one', ierr, one_int)
            return
        end if
        do i = 2, n
            if ( x(i)<=x(i-1) ) then
                ! x-array not strictly increasing.
                ierr = -3
                call xermsg ('PCHIP', 'dpchfd', 'x-array not strictly increasing', ierr, one_int)
                return
            end if
        end do
    end if
    ...
    if ( ne<1 ) then
        ierr = -4
        call xermsg ('PCHIP', 'dpchfd', 'number of evaluation points less than one', ierr, one_int)
        return
    end if
    ierr = 0
    skip = .true.
    ...
    ! *** this case should never occur ***
    ierr = -5
    call xermsg ('PCHIP', 'dpchfd', 'error return from dchfdv -- fatal', ierr, two_int)
...
!     will be counted (at least) twice in the total returned in IERR.
    ...
    real     (kind=RP), intent(in)   :: d(incfd,*) !! array of derivative values.  D(1+(I-1)*INCFD)
                                        !! is the value corresponding to X(I).
    real     (kind=RP), intent(in)   :: xe(*)      !! array of points at which the function is to
                                        !! be evaluated.
                                        !!
                                        !! NOTES:
                                        !!
    ...
    if (.not. skip) then
        if ( n<2 ) then
            ierr = -1
            call xermsg ('PCHIP', 'dpchfe', 'number of data points less than two', ierr, one_int)
            return
        end if
        if ( incfd<1 ) then
            ierr = -2
            call xermsg ('PCHIP', 'dpchfe', 'increment less than one', ierr, one_int)
            return
        end if
        do i = 2, n
            if ( x(i)<=x(i-1) ) then
                ! x-array not strictly increasing.
                ierr = -3
                call xermsg ('PCHIP', 'dpchfe', 'x-array not strictly increasing', ierr, one_int)
                return
            end if
        end do
    end if
    ...
    if ( ne<1 ) then
        ierr = -4
        call xermsg ('PCHIP', 'dpchfe', 'number of evaluation points less than one', ierr, one_int)
        return
    end if
    ...
    ! *** this case should never occur ***
    ierr = -5
    call xermsg ('PCHIP', 'dpchfe', 'error return from dchfev -- fatal', ierr, two_int)
...
!     could conceivably result in IERD=-4, which should be reported.
    ...
    integer  (kind=IB), intent(in)      :: incfd        !! increment between successive values in F and D.
                                            !! (Error return if  INCFD<1).
    integer  (kind=IB), intent(inout)   :: ierr         !! error flag.
                                            !!
                                            !! * Normal return:
                                            !!     * IERR = 0  (no errors).
                                            !! * Warning errors:
    ...
    real     (kind=RP), intent(in)     :: d(incfd,*)   !! array of derivative values.  D(1+(I-1)*INCFD)
                                            !! is the value corresponding to X(I).
    real     (kind=RP), intent(in)     :: a            !! the limits of integration.
    ...
    real     (kind=RP), intent(in)     :: b            !! the limits of integration.
                                            !! NOTE:  There is no requirement that [A,B] be contained in
                                            !!        [X(1),X(N)].  However, the resulting integral value
                                            !!        will be highly suspect, if not.
    logical, intent(inout)   :: skip         !! logical variable which should be set to
    ...
    real     (kind=RP) :: value !! value of the requested integral.

    integer  (kind=IB) :: i, ia, ib_w, ierd, il, ir
    real     (kind=RP) :: xa, xb
    ...
    if (.not. skip) then
        if ( n<2 ) then
            ierr = -1
            call xermsg ('PCHIP', 'dpchia', 'number of data points less than two', ierr, one_int)
            return
        end if
        if ( incfd<1 ) then
            ierr = -2
            call xermsg ('PCHIP', 'dpchia', 'increment less than one', ierr, one_int)
            return
        end if
        do i = 2, n
            if ( x(i)<=x(i-1) ) then
                ierr = -3
                call xermsg ('PCHIP', 'dpchia', 'x-array not strictly increasing', ierr, one_int)
                return
            end if
        end do
    end if
    ...
    if (a /= b) then
        ...
        if (xb <= x(2)) then
            ...
            ! locate ia and ib_w such that
            ! x(ia-1)<xa<=x(ia)<=x(ib_w)<=xb<=x(ib_w+1)
            ia = 1
            do i = 1, n-1
                if (xa > x(i))  ia = i + 1
            end do
            ...
            ! ib_w = n implies xb>x(n) .  otherwise,
            ! ib_w is smallest index such that xb<x(ib_w+1) .

            ! compute the integral.
            if (ib_w < ia) then
                ! this means ib_w = ia-1 and
                ! (a,b) is a subset of (x(ib_w),x(ia)).
                value = dchfie (x(ib_w),x(ia), f(1,ib_w),f(1,ia), &
                                d(1,ib_w),d(1,ia), a, b)
            else

                ! first compute integral over (x(ia),x(ib_w)).
                ! (case (ib_w == ia) is taken care of by initialization
                ! of value to zero.)
                if (ib_w > ia) then
                    value = dpchid (n, x, f, d, incfd, skip, ia, ib_w, ierd)
                    if (ierd < 0) then
                        ! trouble in dpchid.  (should never occur.)
                        ierr = -4
                        call xermsg ('PCHIP', 'dpchia', 'trouble in dpchid', ierr, one_int)
                        return
                    end if
                endif
                ...
                ! then add on integral over (x(ib_w),xb).
                if (xb > x(ib_w)) then
                    ir = min (ib_w+1, n)
                    il = ir - 1
                    value = value + dchfie (x(il),x(ir), f(1,il),f(1,ir), &
                                            d(1,il),d(1,ir), x(ib_w), xb)
                endif
...
!     17, 2 (April 1980), pp. 238-246.
    ...
    integer  (kind=IB), intent(in)  :: ic(2)   !! array of length 2 specifying desired boundary conditions:
                                   !!
                                   !!  * IC(1) = IBEG, desired condition at beginning of data.
                                   !!  * IC(2) = IEND, desired condition at end of data.
                                   !!
    ...
    integer  (kind=IB), intent(in) :: incfd    !! increment between successive values in F and D.
                                   !! This argument is provided primarily for 2-D applications.
                                   !! (Error return if  INCFD<1).
    integer  (kind=IB), intent(in) :: nwk      !! length of work array. (Error return if  NWK<2*(N-1)).
    integer  (kind=IB), intent(out)  :: ierr   !! error flag.
                                   !!
                                   !! * Normal return:
                                   !!
    ...
    real     (kind=RP), intent(in) :: switch   !! indicates desired treatment of points where
    ...
    real     (kind=RP), intent(in) :: x(*)     !! array of independent variable values.  The
                                    ...
                                    !!      X(I-1) < X(I),  I = 2(1)N.
                                    !! (Error return if not.)
    real     (kind=RP), intent(in) :: f(incfd,*) !! array of dependent variable values to be
                                      !! interpolated.  F(1+(I-1)*INCFD) is value corresponding to
                                      !! X(I).
    real     (kind=RP), intent(out) :: d(incfd,*) !! array of derivative values at the data
                                       ...
                                       !!      D(1+(I-1)*INCFD),  I=1(1)N.
                                       !! No other entries in D are changed.
    real     (kind=RP), intent(inout) :: wk(nwk) !! array of working storage.  The user may
    ...
    integer  (kind=IB) :: i, ibeg, iend, nless1

    ! validity-check arguments.
    if ( n<2 ) then
        ierr = -1
        call xermsg ('PCHIP', 'dpchic', 'number of data points less than two', ierr, one_int)
        return
    end if
    if ( incfd<1 ) then
        ierr = -2
        call xermsg ('PCHIP', 'dpchic', 'increment less than one', ierr, one_int)
        return
    end if
    do i = 2, n
        if ( x(i)<=x(i-1) ) then
            ! x-array not strictly increasing.
            ierr = -3
            call xermsg ('PCHIP', 'dpchic', 'x-array not strictly increasing', ierr, one_int)
            return
        end if
    end do
    ...
    if (ierr < 0) then
        ...
        ierr = ierr - 3
        call xermsg ('PCHIP', 'dpchic', 'ic out of range', ierr, one_int)
        return
    end if
    ...
    if ( nwk < 2*nless1 ) then
        ierr = -7
        call xermsg ('PCHIP', 'dpchic', 'work array too small', ierr, one_int)
        return
    end if
    ...
    if (nless1 > 1) then
        ...
        if (switch /= zero) then
            ...
            if (ierr /= 0) then
                ...
                ierr = -8
                call xermsg ('PCHIP', 'dpchic', 'error return from dpchcs', ierr, one_int)
                return
            end if
    ...
    if ( (ibeg/=0) .or. (iend/=0) ) then
        ...
        if (ierr < 0) then
            ...
            ! *** this case should never occur ***
            ierr = -9
            call xermsg ('PCHIP', 'dpchic', 'error return from dpchce', ierr, one_int)
        end if
...
!     calls to [[DCHFIE]].
    ...
    function dpchid (n, x, f, d, incfd, skip, ia, ib_w, ierr) result(value)
      use ModDataType
      implicit none
    ...
    integer  (kind=IB), intent(in)    :: incfd      !! increment between successive values in F and D.
                                        !! (Error return if  INCFD<1).
    integer  (kind=IB), intent(in)    :: ia         !! indices in X-array for the limits of integration.
                                        !! both must be in the range [1,N].  (Error return if not.)
                                        !! No restrictions on their relative values.
    integer  (kind=IB), intent(in)    :: ib_w         !! indices in X-array for the limits of integration.
                                        !! both must be in the range [1,N].  (Error return if not.)
                                        !! No restrictions on their relative values.
    integer  (kind=IB), intent(out)   :: ierr       !! error flag.
                                        !!
                                        ...
                                        !!    * IERR = -2  if INCFD<1 .
                                        !!    * IERR = -3  if the X-array is not strictly increasing.
                                        !!    * IERR = -4  if IA or IB_W is out of range.
                                        !!
                                        !! NOTE: The above errors are checked in the order listed,
                                        !! and following arguments have **NOT** been validated.
    real     (kind=RP), intent(in)   :: x(*)       !! array of independent variable values.  The
    ...
    if (.not. skip) then
        if ( n<2 ) then
            ierr = -1
            call xermsg ('PCHIP', 'dpchid', 'number of data points less than two', ierr, one_int)
            return
        end if
        if ( incfd<1 ) then
            ierr = -2
            call xermsg ('PCHIP', 'dpchid', 'increment less than one', ierr, one_int)
            return
        end if
        do i = 2, n
            if ( x(i)<=x(i-1) ) then
                ! x-array not strictly increasing.
                ierr = -3
                call xermsg ('PCHIP', 'dpchid', 'x-array not strictly increasing', ierr, one_int)
                return
            end if
        end do
    end if
    ...
    ! function definition is ok, go on.
    skip = .true.
    if ((ia<1) .or. (ia>n) .or. (ib_w<1) .or. (ib_w>n)) then
        ! ia or ib_w out of range return.
        ierr = -4
        call xermsg ('PCHIP', 'dpchid', 'ia or ib_w out of range', ierr, one_int)
        return
    end if
...
!     -1 if they are of opposite sign.
    ...
    real     (kind=RP), intent(in)  :: x(n)        !! array of independent variable values.  The
                                        ...
                                        !! `X(I-1) < X(I),  I = 2(1)N`.
                                        !! (Error return if not.)
    real     (kind=RP), intent(in)  :: f(incfd,n)  !! array of dependent variable values to be
                                        !! interpolated.  F(1+(I-1)*INCFD) is value corresponding to
    ...
    real     (kind=RP), intent(out) :: d(incfd,n)  !! array of derivative values at the data
    ...
    integer  (kind=IB) :: i, nless1
    real     (kind=RP) :: del1, del2, dmax, dmin, drat1, drat2, dsave, h1, h2, hsum, hsumt3, w1, w2, tmp

    !f2py integer  (kind=IB), intent(out) :: ierr
    !f2py real     (kind=RP), intent(in) :: x,f
    !f2py real     (kind=RP), intent(out) :: d
    !f2py                     intent(hide), depend(x) :: n=shape(x,0)
    !f2py                     intent(hide), depend(f) :: incfd=shape(f,0),n=shape(f,1)
    !f2py                     intent(hide), depend(d) :: incfd=shape(d,0),n=shape(d,1)

    
    !  validity-check arguments.
    if ( n<2 ) then
        ierr = -1
        call xermsg ('PCHIP', 'dpchim', 'number of data points less than two', ierr, one_int)
        return
    end if
    if ( incfd<1 ) then
        ierr = -2
        call xermsg ('PCHIP', 'dpchim', 'increment less than one', ierr, one_int)
        return
    end if
    do i = 2, n
        if ( x(i)<=x(i-1) ) then
            ! x-array not strictly increasing.
            ierr = -3
            call xermsg ('PCHIP', 'dpchim', 'x-array not strictly increasing', ierr, one_int)
        end if
    end do
...
!  calling, it is unnecessary for such validation to be done here.
    ...
    integer  (kind=IB), intent(in)   :: knotyp
    real     (kind=RP), intent(in)  :: x(*)
    real     (kind=RP), intent(out) :: t(*)

    integer  (kind=IB) :: j, k, ndim
    real     (kind=RP) :: hbeg, hend
...
!@note This is a modified version of C. de Boor's cubic spline routine CUBSPL.
    ...
    integer  (kind=IB), intent(in)     :: ic(2)     !! integer  (kind=IB) array of length 2 specifying desired
                                        !! boundary conditions:
                                        !!
                                        !! * IC(1) = IBEG, desired condition at beginning of data.
                                        !! * IC(2) = IEND, desired condition at end of data.
    ...
    integer  (kind=IB), intent(in)     :: incfd     !! increment between successive values in F and D.
                                        !! This argument is provided primarily for 2-D applications.
                                        !! (Error return if  INCFD<1).
    integer  (kind=IB), intent(in)     :: nwk       !! length of work array. (Error return if NWK<2*N).
    integer  (kind=IB), intent(out)    :: ierr      !! error flag.
                                        !!
                                        !! * Normal return:
                                        !!     * IERR = 0  (no errors).
    ...
    real     (kind=RP), intent(out)   :: d(incfd,*)    !! array of derivative values at the data
                                            ...
                                            !! `D(1+(I-1)*INCFD),  I=1(1)N`.
                                            !! No other entries in D are changed.
    real     (kind=RP), intent(inout) :: wk(2,*)       !! array of working storage.

    integer  (kind=IB) :: ibeg, iend, index, j, nm1
    real     (kind=RP) :: g, stemp(3), xtemp(4)
    logical :: forward

    !  validity-check arguments.
    if ( n<2 ) then
        ierr = -1
        call xermsg ('PCHIP', 'dpchsp', 'number of data points less than two', ierr, one_int)
        return
    end if
    if ( incfd<1 ) then
        ierr = -2
        call xermsg ('PCHIP', 'dpchsp', 'increment less than one', ierr, one_int)
        return
    end if
    do j = 2, n
        if ( x(j)<=x(j-1) ) then
            ! x-array not strictly increasing.
            ierr = -3
            call xermsg ('PCHIP', 'dpchsp', 'x-array not strictly increasing', ierr, one_int)
            return
        end if
    end do
    ...
    if ( ierr<0 ) then
        ! ic out of range return.
        ierr = ierr - 3
        call xermsg ('PCHIP', 'dpchsp', 'ic out of range', ierr, one_int)
        return
    end if
    ...
    if ( nwk < 2*n ) then
        ...
        ierr = -7
        call xermsg ('PCHIP', 'dpchsp', 'work array too small', ierr, one_int)
        return
    end if
    ...
    if ( (ibeg==1).or.(ibeg==2) ) then
        ...
        if (ierr /= 0) then
                ...
                ! *** this case should never occur ***
                ierr = -9
                call xermsg ('PCHIP', 'dpchsp', 'error return from dpchdf', ierr, one_int)
        end if
        ibeg = 1
    endif
    ...
    if (nm1 > 1) then
        do j=2,nm1
            if (wk(2,j-1) == zero) then
                ...
                ierr = -8
                call xermsg ('PCHIP', 'dpchsp', 'singular linear system', ierr, one_int)
                return
            end if
            g = -wk(1,j+1)/wk(2,j-1)
...
! possible over/underflow problems.
    ...
    real     (kind=RP), intent(in) :: arg1
    real     (kind=RP), intent(in) :: arg2
    real     (kind=RP) :: s
...
!@warning This routine does no validity-checking of arguments.
    ...
    integer  (kind=IB), intent(in)  :: iextrm       !! index of the extreme data value.  (assumes
    ...
    real     (kind=RP), intent(in) :: dfmax        !! maximum allowed difference between F(IEXTRM) and
                                        !! the cubic determined by derivative values D1,D2.  (assumes
                                        !! DFMAX>0.)
    real     (kind=RP), intent(inout) :: d1        !! derivative values at the ends of the interval.
                                        !! (Assumes D1*D2 <= 0.)
                                        !! (output) may be modified if necessary to meet the restriction
                                        !! imposed by DFMAX.
    real     (kind=RP), intent(inout) :: d2        !! derivative values at the ends of the interval.
                                        !! (Assumes D1*D2 <= 0.)
                                        !! (output) may be modified if necessary to meet the restriction
                                        !! imposed by DFMAX.
    real     (kind=RP), intent(in) :: h            !! interval length.  (Assumes  H>0.)
    real     (kind=RP), intent(in) :: slope        !! data slope on the interval.

    real     (kind=RP) :: cp, hphi, nu, radcal, sigma
    real     (kind=RP) :: lambda  !! the ratio of d2 to d1.
    real     (kind=RP) :: phi     !! phi is the normalized value of p(x)-f1 at x = xhat = x-hat(rho),
                        !! where  that = (xhat - x1)/h .
                        !! that is, p(xhat)-f1 = d*h*phi,  where d=d1 or d2.
                        !! similarly,  p(xhat)-f2 = d*h*(phi-rho) .
    real     (kind=RP) :: rho     !! the ratio of the data slope to the derivative being tested.
    real     (kind=RP) :: that    !! that = t-hat(rho) is the normalized location of the extremum.

    real     (kind=RP), parameter :: fact = 100.0_RP
    real     (kind=RP), parameter :: third = one/three - d1mach4  !! third should be slightly less than 1/3 (original code had 0.33333)
    real     (kind=RP), parameter :: small = fact*d1mach4 !! small should be a few orders of magnitude greater than macheps.
    ...
    if (d1 == zero) then
        ...
        if (d2 == zero) then
            ...
            ierr = -1
            call xermsg ('PCHIP', 'dpchsw', 'd1 and/or d2 invalid', ierr, one_int)
            return
        end if
        ...
        if (d2 == zero) then
            ...
            if (abs(nu) > small) then
                ...
                if (radcal < zero) then
                    ...
                    ierr = -2
                    call xermsg ('PCHIP', 'dpchsw', 'negative radical', ierr, one_int)
                end if
...
!  A very simplified version of the SLATEC routine.
    ...
    subroutine xermsg(librar,subrou,messg,nerr,level)
      ...
      integer  (kind=IB), intent(in) :: nerr   !! error code
      integer  (kind=IB), intent(in) :: level  !! value in the range 0 to 2 that indicates the
