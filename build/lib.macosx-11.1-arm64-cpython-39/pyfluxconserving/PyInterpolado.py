...
RESUME :  Interpolation script calling Interpolado in Fortran!
...
Total number of arguments = 9                                         
   ...
   INPUT  : 01) xx_value   -> Interpolate into x array                            
                  ...
                  07) verbosity   -> Print & Check screen (OLD fortran way)              
...
author_interpolado in python
######################################################################  

######################################################################             
                 HOW TO USE
                 
                 i_object = Interpolado
                 i_object.Interpolado(x_new,x,y,[Is_Index],[verbosity])
                 ...
                 [Is_Index]   => Search: Linear: [1] & Binary [0]
...
"""  
    ...
    def __init__(self,x_new,x,y,Is_Index=0,verbosity=0):
        self.x = x
        self.y = y
        self.x_new = x_new
        ...
        self.y_new, self.IsKeepOn = interp.interpolado(self.x_new,self.x,is_index=self.Is_Index,verbosity=self.verbosity)
    ...
    def TestRoutine():
        ...
        y_interp,IsKeepOn = interp.interpolado(x_interp,x,y,verbosity=verbosity)
        y_new_interp,IsKeepOn = interp.interpolado(x_new_interp,x_new,y_new,verbosity=verbosity)
        ...
        ax1_top.plot(x_interp, y_interp, color='blue',label='Interpolado Interpolation',linewidth=10,alpha=0.3)
        ...
        # Interpolated function Fortran - f2py
        ax2_top.plot(x_new_interp, y_new_interp, color='blue',label='Interpolado Interpolation',linewidth=10,alpha=0.3)
    ...
    def time():
        ...
        for i in enumerate(val_array):
            ...
            for pop in range(0,population):
                ...
                y_interp,IsKeepOn = interp.interpolado(x_interp,x,y,is_index=Is_Index,verbosity=verbosity)
