...
RESUME : Spline1DArr subroutine is written in fortran 2003 standard.
...
######################################################################
SUBROUTINE SPLINE1DArr( x,o,m,t,y,n,e,IsKeepOn,k )
spline1darr in python
...
author_spline1darr in python
######################################################################  

######################################################################             
                 HOW TO USE
                 
                 i_object = SPLINE1DArr
                 i_object.SPLINE1DArr(x_new,x,y,[e],[verbosity])
                 ...
                 [e]               => Machine precision e=1.0e-8 by default
...
"""  
    ...
    def __init__(self,x_new,x,y,e=1.0e-8,Is_Index=0,verbosity=0):
        self.x = x
        self.y = y
        self.x_new = x_new
        ...
        self.y_new, self.IsKeepOn = interp.spline1darr(self.x_new,self.x,self.y,e=self.e,Is_Index=self.Is_Index,verbosity=self.verbosity)
    ...
    def TestRoutine():
        ...
        y_interp,IsKeepOn = interp.spline1darr(x_interp,x,y,verbosity=verbosity)
        y_new_interp,IsKeepOn = interp.spline1darr(x_new_interp,x_new,y_new,verbosity=verbosity)
        ...
        ax1_top.plot(x_interp, y_interp, color='blue',label='SPLINE1DArr Interpolation',linewidth=10,alpha=0.3)
        ...
        # Interpolated function Fortran - f2py
        ax2_top.plot(x_new_interp, y_new_interp, color='blue',label='SPLINE1DArr Interpolation',linewidth=10,alpha=0.3)
    ...
    def time():
        ...
        for i in enumerate(val_array):
            ...
            for pop in range(0,population):
                ...
                print("... Time    Spline1DArr: {0:} s ".format(auxil2time-auxil1time))
        ...
        plt.plot(sizepoints,timearray_average[:,0],color='darkorange',label='Time SPLINE1DArr Fortran', zorder=1)
...
def main():
    ...
    #i_object = SPLINE1DArr(x_new,x,y)
